...
def seed_shared_dir(
    ...
    *,
    ...
    paths.daemon_pid_file.write_text(str(daemon_pid))
...
def test_first_owner_uses_native_pid_file_and_returns_one_lease(short_root):
    ...
    assert manager.paths.daemon_pid_file.read_text() == str(client.daemon_pid)
...
def test_unexpected_clients_on_fresh_daemon_are_preserved(short_root):
    client = FakeControlClient()
    original_start = client.start_daemon

    def start_with_foreign_client(pipe_dir, log_dir, gpu_uuid):
        original_start(pipe_dir, log_dir, gpu_uuid)
        client.set_clients(pipe_dir, {7000: [31337]})

    client.start_daemon = start_with_foreign_client
    manager = make_manager(short_root, client)

    with pytest.raises(MpsError, match="already reports clients"):
        manager.acquire()
...
def test_probe_checks_daemon_and_retained_clients(short_root):
    client = FakeControlClient()
