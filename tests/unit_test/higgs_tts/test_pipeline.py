    ...
    request = SimpleNamespace(
        data=SimpleNamespace(
            ...
            reference_codes_delayed=torch.tensor(
                [[10, 11], [20, 21], [30, 31]], dtype=torch.long
            ),
        )
    )
    forward_batch = SimpleNamespace(
...
def test_higgs_model_runner_prefill_slices_chunks_by_prefix_position() -> None:
    ...
    # Chunk two resumes after 3 prompt tokens, of which 2 were placeholders,
    # so this chunk must start at reference row 2.
...
def test_higgs_model_runner_prefill_skips_fully_cached_reference() -> None:
    ...
    data = SimpleNamespace(
        req=SimpleNamespace(
            origin_input_ids=_PREFILL_PROMPT_IDS.tolist(),
            extend_range=SimpleNamespace(start=7, length=1),
        ),
        input_ids=_PREFILL_PROMPT_IDS,
        reference_codes_delayed=_PREFILL_REF_CODES,
    )
